...
class Coupon(Base):
    ...
    discount_type = Column(Enum(couponenums.DiscountType, values_callable=lambda x: [e.value for e in x]), nullable=False)
    value = Column(Float, nullable=True) 
    buy_count = Column(Integer, nullable=True)
    get_count = Column(Integer, nullable=True)
    is_active = Column(Boolean, default=True)
    applies_to_entity = Column(Enum(couponenums.CouponScope, values_callable=lambda x: [e.value for e in x]), default=couponenums.CouponScope.GLOBAL)
