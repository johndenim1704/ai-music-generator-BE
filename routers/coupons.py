...
def create_coupon(request: CouponCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if request.discount_type.value == "percent":
            ...
            # Stripe doesn't support B1G1; 1% placeholder (0% rejected by Stripe)
            stripe_coupon_params['percent_off'] = 1
        ...
        try:
            ...
            # Duplicate promo code on Stripe — clean up the coupon we just created
            if stripe_coupon:
                stripe.Coupon.delete(stripe_coupon.id)
            raise HTTPException(status_code=400, detail=f"Coupon code '{request.code}' already exists on Stripe. Please use a different code or delete the existing one first.")
        ...
        # DB save failed — roll back and clean up Stripe objects to avoid orphans
        db.rollback()
        if stripe_promo_code:
            try:
                stripe.PromotionCode.modify(stripe_promo_code.id, active=False)
            except Exception:
                pass
        if stripe_coupon:
            try:
